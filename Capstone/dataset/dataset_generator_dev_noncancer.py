...
def get_mask(coords):
    ...
    x, y = np.mgrid[minimum:maximum, minimum:maximum]
    #print("Done generating meshgrid!")
    #print("Flattening")
...
def generate_data(data,mode,ltype):
    ...
    for slide in data:
        ...
        #pdb.set_trace()
        outfile = os.path.join(save_path,mode,"nc_"+str(slide))
        try:
...
cancer, noncancer = get_statistics(test)


# In[ ]:


#Generate Non Cancerous Patches for dev
threads = []
batch = 0
slide_count = 0
slide_list = os.listdir(os.path.join(save_path,'dev'))
print(slide_list)
temp_dict = {}

for slide in noncancer:
    if "nc_"+str(slide)+".npz" not in slide_list and slide!=81063 :
    	...
    	if batch == 5:
        	batch=0
        	print(len(temp_dict))
        	t = threading.Thread(target=generate_data, args=(temp_dict,'dev','noncancer',))
